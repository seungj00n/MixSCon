...
- Backbone: ResNet50 (optional timm backbone)
- Loss: SCL + beta*Mixup-SCL + gamma*CE
- 2-view augmentation (train only)
- Remove self-similarity (diagonal elements) from numerator and denominator

Saved artifacts:
- best_model_round_X.pth        : best validation checkpoint (including optimizer state)
- final_weights_round_X.pth     : final state_dict
- checkpoint_round_X.pth        : full checkpoint per round
- confusion_matrix_round_X.png, training_history_round_X.png
- results_round_X.json
- best_overall_model.pth        : best model among 5 folds based on test accuracy
- cross_validation_results.json : cross-validation summary
"""
...
CONFIG = {
    ...
    'weight_decay': 1e-4,
    'temperature': 0.07,
    'beta': 1.0,   # Mixup-SCL weight
    'gamma': 1.0,  # CE weight
    # Model
    'backbone': 'resnet50',
    'use_timm': False,  # set True to use timm backbones (e.g., convnext_base etc.)
    'proj_dim': 128,
    # Augmentations
    'two_views': True,   # 2-view augmentation for training
...
class TwoViewDataset(Dataset):
    """Wrap ImageFolder and return two independently augmented views from the same source image"""
...
def build_train_transforms(cfg: Dict):
    ...
    return t, t   # identical policy for both views
...
def make_loaders_for_round(round_dir: Path, test_dir: Path, cfg: Dict) -> Tuple[DataLoader, DataLoader, DataLoader, int, List[str]]:
    """
    round_dir: directory containing train/val for a given fold
    test_dir : common test dataset directory
    """
...
class End2EndBackbone(nn.Module):
    ...
    def _forward_feat(self, x):
        ...
        x = self.avgpool(x); return torch.flatten(x, 1)
...
def validate(model, loader, device, num_classes, beta, gamma, temperature):
    """
    Validation uses CE loss only (contrastive term removed)
    """
    model.eval()
    total_loss = total_con = total_ce = 0.0
    correct = n = 0
...
class EarlyStopping:
    """
    Save deep-copied best weights safely
    """
...
def run_cross_validation(cv_root: str, test_root: str, save_root: str, cfg: Dict):
    ...
    with open(save_root / 'cross_validat
